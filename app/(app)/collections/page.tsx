...
export default function CollectionsPage() {
  ...
  const filteredProducts = useMemo(() => {
    const filtered = sampleProducts.filter((product) => {
      ...
      // Style filter
      if (selectedStyles.length > 0 && !product.style.some((s: string) => selectedStyles.includes(s))) return false;

      // Color filter
      if (selectedColors.length > 0 && !product.color.some((c: string) => selectedColors.includes(c))) return false;

      // Material filter
      if (selectedMaterials.length > 0 && !product.material.some((m: string) => selectedMaterials.includes(m))) return false;
